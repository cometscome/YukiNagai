...
[Green関数とは](notes/note_0916_green.pdf) 
Green関数を用いると物理量が計算できるらしい。 
(2006年1月21日、間違いを訂正。) 
(2006年9月6日、細かなミスを訂正。) 

[Green関数の求めかた](notes/note_0923_green.pdf) 
...
[密度行列演算子の時間発展と物理量の期待値](notes/note_070124_co.pdf) 
...
[Keldysh形式の非平衡Green関数の導入](notes/note_070605_kel.pdf) 
...
[非線形方程式の数値解法：自己無撞着な方程式を解く](notes/note_070420_self.pdf) 
...
[松原Green関数の高速フーリエ変換](notes/note_090424_FFT.pdf) 
高速フーリエ変換(FFT)のアルゴリズムについて軽くまとめた。 
また、松原Green関数のフーリエ変換をFFTに載せるための変形について書いた。 

[有限サイズモデルでバルクを扱う方法](notes/note_111224_finiteTB.pdf) 
ハミルトニアンが二次形式で表現される有限サイズモデルを使って、
バルクの系を計算する方法について。 
エネルギー方向の解像度が欲しいときに行う方法。 

[Runge-Kutta法による一階常微分方程式の解法](notes/note_131119_RungeKutta.pdf) 
準古典理論で現れる方程式であるRIccati方程式は一階常微分方程式である。この微分方程式を数値的に解く為には、Runge-Kutta法が使われる。 
8段7次のRunge-Kutta法の係数についてまとめた。 

[Sakurai-Sugiura法による行列の対角化](notes/note_140502_SS.pdf) 
Sakurai-Sugiura法(SS法)は、任意の範囲の固有値と固有ベクトルを求める事のできる手法である。
この手法はプログラムが比較的単純にも関わらず高速に高精度に計算ができる。
 
この手法の解説を行う。 

[Sakurai-Sugiura 法による固有値問題ソルバー z-Pares の解説](notes/note_140903_SSzpares.pdf) 
Sakurai-Sugiura法(SS法)による固有値問題ソルバーz-Paresが[公開](http://zpares.cs.tsukuba.ac.jp)された。
このノートはそのインストールと実行に関する日本語解説である。  
密行列用サンプルコード逐次版は[こちら](notes/zpares_sub.f90)。MPI並列版は[こちら](notes/zpares_sub_mpi.f90)。
 
疎行列用サンプルコードMPI版は[こちら](notes/zpares_subCRS_mpi.f90)。 
行列の(i,j)における要素vを計算するサブルーチンを用意すれば勝手にCRS形式に変換して計算できるようにした。
 
[固有値ソルバLOBPCG法の解説](notes/note_141009_LOBPCG.pdf) 
...
サンプルコードは[こちら](notes/test_LOBPCG.f90)。
 
[Hubbard模型の厳密対角化](notes/note_170223_exactD.pdf) 
Hubbard模型を厳密対角化で解く手法について述べる。
簡単のため、一次元を考える。 
厳密対角化で重要なことは解くべきハミルトニアンを行列表示することである。 
このノートでは主に行列表示する方法について述べる。 
サンプルコードは[こちら](notes/exact.py)。
Python 2系で書いた。なお、このコードは二次元Hubbard模型の最小固有値を求めることができる。
 
[物性物理で使える誰でもできるMPI並列計算の初歩](notes/note_170705_MPI.pdf) 
...
サンプルコードは[こちら](notes/MPItest.zip)。
 

[数値計算で学ぶ量子力学: プログラミング言語Juliaによる数値計算入門](notes/JuliaQM.pdf) 
プログラミング言語Juliaを使って、量子力学を解いてみた。現状まだ未完成。 
JuliaはFortranやCに匹敵する速度で実行できるにも関わらず、行列の対角化や数値積分などが非常に簡便に実行できる。 
詳しくは[こちら(外部サイト)](https://qiita.com/cometscome_phys/items/31d0b811345a3e12fcef)も参考にすると良い。 

[物理で使う数値計算入門：Julia言語による簡単数値計算](notes/julianote.pdf) 
FortranやC並みに高速かつPythonのように書きやすい数値計算向きプログラミング言語Juliaの解説。 
随時更新中。最新版は[こちら(外部サイト)](https://github.com/cometscome/Julianotes)にある
...
強相関電子系関連のノート。  
[ハバードモデルのSlave boson平均場近似によるアプローチ](notes/note_070709_slave.pdf) 
...
[Green関数のエネルギー−運動量表示と準粒子描像](notes/note_071017_momen.pdf) 
Green関数のエネルギー−運動量表示を求めた。またその表示を用いて、素励起について考えた。 
また、自己エネルギーを導入してDyson方程式を導いてみた。 

[Anderson模型のHartree-Fock近似](notes/note_071023_anderson.pdf) 
...
[経路積分表示でのAnderson模型のHartree-Fock近似](notes/note_080408_anpath.pdf) 
経路積分表示でのAnderson模型の有効作用を求めた。 
結局のところグラスマン数の積分を行っただけのノートである。 
DMFTの下準備ノート。  

[Hubbard模型の有効作用と動的平均場理論](notes/note_080411_aneff.pdf) 
...
- [3.1 相互作用のない古典気体](notes/chaikin_3_1.pdf) 
- [4.2 外場中の反強磁性体の三重臨界点](notes/chaikin_4_2.pdf) 
- [5.3 異方的な系の相関関数](notes/chaikin_5_3.pdf)
...
- [3.9](notes/resume_rinko_3-9.pdf) 
- [3.11](notes/resume_rinko_3-11.pdf) 
- 4 GINZBURG-LANDAU THORY 
- [4.3-4.5](notes/resume_rinko_4-3_12_20.pdf) 
- [4.10-4.11](notes/resume_rinko_4-10.pdf)


---

**講義ノート**
[超伝導の基礎](notes/superconductivity2023.pdf) 
学部３年生むけの超伝導の講義ノートです。講義の補助として使う都合上、図はありません。
